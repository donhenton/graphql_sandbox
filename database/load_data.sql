...
update  author set tokens = to_tsvector(name);
...
insert into  book(id, google_id, title, subtitle, description, page_count, rating_total, rating_count, rating ) values
...
-- update tokens column for full text search
update  book set tokens = to_tsvector(coalesce(title, '') || coalesce(subtitle, '') || coalesce(description, ''));
...
-- reset next sequence used for id column.
alter sequence  book_author_id_seq restart with 4;

insert into  user_list(id, email, name) values 
  (1, 'james@knowthen.com', 'knowthen'),
  (2, 'joe@mail.com', 'joe'),
  (3, 'jane@mail.com', 'jane');

-- update tokens column for full text search
update  user_list set tokens = to_tsvector(name);

-- reset next sequence used for id column.
alter sequence  user_list_id_seq restart with 4;

insert into  review(id, user_id, book_id, rating, title, comment) values 
...
-- update tokens column for full text search
update  review set tokens = to_tsvector(coalesce(title, '') || coalesce(comment, ''));

-- reset next sequence used for id column.
alter sequence  review_id_seq restart with 7;
